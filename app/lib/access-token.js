...
export class AccessToken {
  ...
  async requestAccessToken({ grantType = this.grantType } = {}) {
    ...
    if (this.proxy) {
      ...
      console.debug('== Making request through a proxy')

      return new Promise((resolve, reject) => {

        const url = new URL(this.tokenHost + this.tokenPath)
        url.searchParams.set('grant_type', grantType)
        url.searchParams.set('scope', normalizeScope(this.tokenConfig.scope))

        const execArgs = `curl --max-time ${this.httpClientTimeout / 1000} --proxytunnel ${this.proxy ? `-x ${this.proxy}` : ``} -X POST -u ${this.wskey}:${this.wskeySecret} "${this.tokenHost}${this.tokenPath}?grant_type=${grantType}&scope=SCIM"`
          ...
          if (stdout) {
            ...
            try {
              ...
              Object.keys(result).forEach(key => {
                this[camelCase(key)] = result[key];
                ret[camelCase(key)] = result[key];
              })
              return resolve(ret)
        ...
        axios.post(this.tokenHost + this.tokenPath, {}, {
          ...
          .then(response => {
            if (response.status === 200) {
              console.debug('Success: Got a token')
              // Success
  ...
  async _doCreate() {
    ...
    return new Promise((resolve, reject) => {
      try {
        const token = this.requestAccessToken();
        resolve(token)
      } catch (e) {
        reject(e)
      }
